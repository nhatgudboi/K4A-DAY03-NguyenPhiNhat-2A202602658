        ...
        # Trường hợp 2: LLM đề xuất gọi Tool (Action)
            ...
            if not obs_data:
                ...
                if obs_data.get("status") == "SUCCESS":
                    if tool_name == "doctor_schedule_query" and "data" in obs_data:
                        d = obs_data["data"]
                        slots = ", ".join(d.get("available_slots", []))
                        final_answer = (
                            f"Lịch làm việc của chuyên khoa {obs_data.get('specialty', '')}: "
                            f"Bác sĩ {d.get('doctor_name', '')}, Khung giờ: {slots}, "
                            f"Phòng: {d.get('room', '')}."
                        )
                    elif tool_name == "book_health_check":
                        final_answer = obs_data.get("message", "Đặt lịch thành công.")
